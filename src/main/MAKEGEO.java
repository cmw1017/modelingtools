...
public class MAKEGEO {
	...
	public void mainP(int continueNum, String inputName, Scanner sc) throws IOException, InterruptedException {
		String LU = "", Terrian = "", OutGEO = "", ListGEO = "", LUGRID = "", TerrianGRID = "", insrc;
		...
		System.out.print("설정 파일의 확장자 명을 포함한 파일 전체 이름 입력하여 주세요(ex:makegeo.set) : ");
		sc.nextLine();
		insrc = sc.nextLine();
		if(!insrc.substring(insrc.indexOf('.')+1).equals("set")) {
			System.out.println("확장자를 확인하여 주세요(set 형식이어야 합니다).");
			return;
		}
		insrc = ".\\input\\" + insrc;
		...
		inStream.close();
		String outsrc = ".\\output\\makegeo.inp";
		String tempstr;
		...
		tempstr = "MAKEGEO.INP     2.0             Snow Processing\r\n"
				...
				+ "     LU2.DAT       input    ! LU2DAT  = !\r\n" + "     TERR.DAT      input    ! TERRDAT =" + Terrian
		...
		File LUfile = new File(".\\input\\" + LU);
		File Terrfile = new File(".\\input\\" + Terrian);
		if (!LUfile.isFile()) {
			System.out.println("No such file here(" + LU + ")");
			return;
		}
		if (!Terrfile.isFile()) {
			System.out.println("No such file here(" + Terrian + ")");
			return;
		}
		
		try {
			process = new ProcessBuilder("cmd", "/c", "copy", ".\\input\\" + Terrian, ".\\.setting\\MAKEGEO").start();
			process = new ProcessBuilder("cmd", "/c", "copy", ".\\input\\" + LU, ".\\.setting\\MAKEGEO").start();
			process = new ProcessBuilder("cmd", "/c", "copy", ".\\output\\makegeo.inp", ".\\.setting\\MAKEGEO").start();
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			process.waitFor();
		...
		File make = new File(".setting\\MAKEGEO\\makegeo.exe");
		File makedir = new File(".setting\\MAKEGEO");
		...
		if (make.isFile()) {
			try {
				process2 = new ProcessBuilder("cmd", "/c", "start", makesrc);
			...
			System.out.println("MAKEGEO 프로그램이 설치 되어있지 않거나 변경하였습니다(makegeo.exe)");
			return;
		}
	}
}
