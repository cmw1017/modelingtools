...
public class MAKEResultPanel extends JFrame implements PanelTemplete {
	...
	// 페이지 이동
		...
		public void make_logic(String load_path) throws IOException, InterruptedException {
			...
			File LUfile = new File(root+"\\MAKEGEO\\" + LU);
			File Terrfile = new File(root+"\\MAKEGEO\\" + Terrian);
			if (!LUfile.isFile()) {
			...
			try {
				process = new ProcessBuilder("cmd", "/c", "copy", root + Terrian,  root +"\\MAKEGEO").start();
			...
			File make = new File(root+"\\MAKEGEO\\makegeo.exe");
