...
public class AERMAIN implements Runnable{
	...
	public void run() {
		try {
			...
			while(queue.size() != 0 || !threadAllDone) {
				...
				if(t_info.current_thread_count == 0)
					threadAllDone = true;

				System.out.println("CPU usage : " + currentCpuLoad);
				System.out.printf("WAIT_current thread count : %d / queue size : %d / threadAllDone : %s\n"
				,t_info.current_thread_count
				,queue.size()
				,threadAllDone ? "True" : "False");
				for(int i = 0; i < max_thread; i++) {
			...
			System.out.println("END_Queue And Current Model is empty");
